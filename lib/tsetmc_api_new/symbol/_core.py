...
def get_symbol_price_overview(symbol_id: str) -> dict:
    ...
    traders_type = {
        'legal': {
            'buy': {
                'value': l_buy_v * price_data['close'],
                'volume': l_buy_v,
                'count': l_buy_c,
            },
            'sell': {
                'value': l_sell_v * price_data['close'],
                'volume': l_sell_v,
                'count': l_sell_c,
            }
        },
        'real': {
            'buy': {
                'value': r_buy_v * price_data['close'],
                'volume': r_buy_v,
                'count': r_buy_c,
            },
            'sell': {
                'value': r_sell_v * price_data['close'],
                'volume': r_sell_v,
                'count': r_sell_c,
            }
        },
...
def get_symbol_major_shareholders(symbol_id: str) -> list[dict]:
    response = requests.get(
        url='http://tsetmc.ir/Loader.aspx',
        params={
            'i': symbol_id,
    ...
    for tr in trs:
        ...
        count = locale.atoi(tds[1].div['title'])
