...
def _convert_heven_to_jtime(heven: str) -> jtime:
    heven = str(heven)
    if len(heven) == 6:
...
def get_day_details_price_data(symbol_id: str, date: jdate) -> list[dict]:
    ...
    price_data = [{
        'time': _convert_heven_to_jtime(heven=row['hEven']),
...
def get_day_details_orderbook_data(symbol_id: str, date: jdate) -> list[dict]:
    ...
    for row in response:
        heven = row['hEven']
        t = _convert_heven_to_jtime(heven=heven)
    ...
    return [{
        'time': _convert_heven_to_jtime(heven=key),
        **value,
    } for key, value in heven_map.items()]


def get_day_details_trade_data(symbol_id: str, date: jdate) -> list[dict]:
    t = date.togregorian().strftime('%Y%m%d')
    response = requests.get(
        url=f'http://cdn.tsetmc.com/api/Trade/GetTradeHistory/{symbol_id}/{t}/false',
        params={},
        verify=False,
    )
    response.raise_for_status()
